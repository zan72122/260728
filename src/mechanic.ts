...
export function drawRobotLying(ctx: Ctx, o: LyingOpts): void {
  ...
  if (o.lampOn > 0) {
    ...
    const bx = lampX + aim * s * 7;
    const reachY = lampY - s * 10;
    const g = ctx.createRadialGradient(lampX, lampY, s * 0.1, bx, reachY, s * 8.5);
    g.addColorStop(0, `rgba(255,238,180,${0.45 * o.lampOn})`);
    g.addColorStop(1, 'rgba(255,238,180,0)');
    ctx.fillStyle = g;
    ctx.beginPath();
    ctx.moveTo(lampX - s * 0.35, lampY);
    ctx.lineTo(bx - s * 3.2, reachY);
    ctx.lineTo(bx + s * 3.2, reachY);
